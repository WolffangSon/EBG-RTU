...
chrome.action.onClicked.addListener((tab) => {
  chrome.storage.local.get(
    ["webappURL", "sheetTab", "botActive", "refreshMins"],
    (result) => {
      ...
      chrome.storage.local.get(
        ...
        (result) => {
          if (result.botActive == 1) {
            ...
            console.log("Warning: RTU Bot is inactive! Nothing to do.");
          } else {
...
function activateBot(tabId, alarmName, refreshMins) {
  chrome.tabs.sendMessage(tabId, { type: "activate" }, (response) => {
    if (chrome.runtime.lastError) {
      botSetIcon("warning", "RTU Bot (Not loaded)");
      botNotifSend(
        "warning",
        "Script not loaded!",
        "Please refresh the page or make sure you have F9 Dashboard open and try again."
      );
    } else {
      setBotAlarm(alarmName, refreshMins);
      chrome.storage.local.set({ botActive: 1 }, () => {
        botSetIcon("activated", "RTU Bot (Active)", {
          text: `${refreshMins}`,
          color: "green",
        });
        botNotifSend(
          "activated",
          "RTU Bot Activated!",
          `The Bot is updating every ${refreshMins} minutes.`
        );
        console.log("RT Bot Activated!");
      });
    }
  });
}
...
function deactivateBot(alarmName) {
  ...
  chrome.storage.local.set({ botActive: 0 }, () => {
    ...
    botNotifSend("deactivated", "RTU Bot Deactivated!", "No more updates.");
    console.log("RTU Bot Inactive!");
  });
}
...
function setBotAlarm(name, minutes) {
  ...
  console.log(
    "RTU Bot Alarm: " + name + " has been set every: " + minutes + " minutes"
  );
}
...
function unsetBotAlarm(name) {
  chrome.alarms.clear(name, (wasCleared) => {
    if (wasCleared == true) {
      console.log("RTU Bot Alarm: " + name + ". Was unset.");
    } else {
      console.log(
        "RTU Bot Error: There was an issue unsetting the alarm: " + name
      );
    }
  });
}
    ...
    if (chrome.runtime.lastError) {
      chrome.storage.local.get(["errorCounter"], (result) => {
        if (result.errorCounter > 4) {
          ...
          botNotifSend(
            "warning",
            "F9 Data Not Processed!",
            `Deactivating F9 RTU Bot due to ${result.errorCounter} consecutive failures.`
          );
          ...
          botNotifSend(
            "warning",
            "Data Not Processed!",
            "Please reload or make sure you have F9 Dashboard open and try again."
          );
          ...
          chrome.storage.local.get(["errorCounter"], (result) => {
            if (result.errorCounter > 4) {
              ...
              botNotifSend(
                "warning",
                "Data Not Processed!",
                `Deactivating F9 RTU Bot due to ${result.errorCounter} consecutive POST failures.`
              );
              chrome.storage.local.set({ errorCounter: 0 });
            } else {
              botNotifSend(
